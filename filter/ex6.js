...
const bannedTab = ['bloody', 'damn'];
function filterOffensiveComments(comments, bannedWords) {
  const decentSetences = comments.filter(function(comment){
    return !(comment.toLowerCase().includes(bannedWords[0]) || comment.toLowerCase().includes(bannedWords[1])) ;
  })
  return decentSetences;
}
console.log(filterOffensiveComments(commentTab, bannedTab));
